...
//!
//! This is *honest* resolution, not evasion: it asks a truthful public resolver
//! by name instead of trusting a local resolver that is lying, and the query
//! rides the same OS-trust HTTPS client as the rest of b2p (so a TLS-inspecting
//! proxy with its CA supplied via `--cafile` passes it rather than breaks it).
//! It only runs on the `--tunnel` path; the default WebRTC transport never
//! resolves a tunnel host at all.
...
/// The DoH client trusts the OS store plus any `--cafile` root, so the query
/// survives a TLS-inspecting proxy the same way b2p's other HTTPS traffic does.
pub async fn resolve(host: &str, tls: &TlsOpts) -> anyhow::Result<Vec<IpAddr>> {
    let client = crate::http::builder(tls)?
...
mod tests {
    ...
    async fn live_resolves_via_real_doh() {
        let ips = resolve("blog.cloudflare.com", &TlsOpts::default())
            .await
            .unwrap();
